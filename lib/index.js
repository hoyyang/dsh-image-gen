...
import { DEFAULT_IMAGES_PATH, DEFAULT_MODEL, DEFAULT_OUTPUT_DIRECTORY, DEFAULT_QUALITY, DEFAULT_SIZE, DEFAULT_TIMEOUT_MS, QUALITIES, SIZES, checkConfiguration, generateImage, sanitizeError, } from './image-service.js';
...
function resolveConfig(cordis, settings = {}, file = {}) {
    ...
    const outputDirectory = firstNonEmpty(env('DSH_IMAGE_OUTPUT_DIR'), settings.outputDirectory, cordis.outputDirectory, file.outputDirectory) || DEFAULT_OUTPUT_DIRECTORY;
...
const QUALITY_OPTIONS = [...QUALITIES];
function parseCommandInput(rawInput) {
    ...
    if (!prompt) {
        return {
            prompt: '',
            error: 'Usage: /dsh-image-gen <prompt> [--size=1024x1024|1024x1536|1536x1024|auto] [--quality=low|medium|high]',
        };
    }
...
export function apply(ctx, config) {
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        description: '本地检查图像生成配置（网关 baseUrl / API Key / 输出目录），不发网络请求、不产生账单。不确定配置是否健康时先调用。',
        parameters: {},
        output: {
            schema: {
                type: 'object',
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        name: 'dsh_image_gen',
        description: '付费且非幂等：调用配置的 OpenAI 兼容生图网关生成一张图并保存到本地。单次上游请求，绝不自动重试。',
        parameters: {
        ...
        async execute(args) {
            try {
                const result = await generateImage({ prompt: args.prompt, size: args.size ?? DEFAULT_SIZE, quality: args.quality ?? DEFAULT_QUALITY }, resolved);
    ...
    ctx.effect(() => ctx.commands.register({
        ...
        handler: async (invocation) => {
            ...
            try {
                const result = await generateImage({ prompt: parsed.prompt, size: parsed.size, quality: parsed.quality }, resolved, globalThis.fetch, invocation.signal);
                return {
                    kind: 'success',
                    text: [
                        'Generated one image.',
    ...
    }), 'dsh-image-gen: command');
    ctx.effect(() => ctx.skills.register({
        name: 'dsh-image-gen',
        source: 'runtime',
        description: 'Generate a raster image through the configured OpenAI-compatible image gateway when the user asks to create an image.',
        content: [
...
function resolveEndpointText(config) {
