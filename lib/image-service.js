...
export function resolveEndpoint(config) {
    ...
    if (!base) {
        throw new Error('No gateway base URL configured. Set it in DSH settings (Plugins page), in the profile config row, in ~/.dsh/dsh-image-gen.json, or via DSH_IMAGE_BASE_URL.');
    }
...
export function readApiKey(config) {
    ...
    if (apiKey.length < 8) {
        throw new Error('API key missing. Set it in DSH settings (Plugins page), in the profile config row, in ~/.dsh/dsh-image-gen.json, or via DSH_IMAGE_API_KEY.');
    }
    return apiKey;
}
...
export async function checkConfiguration(config) {
...
function extractUpstreamError(payload, status, apiKey) {
    const rawMessage = payload?.error?.message ?? payload?.message ?? 'The image service returned HTTP ' + status + '.';
...
async function writeImageAtomically(outputDirectory, buffer, extension) {
...
export async function generateImage(args, config, fetchImpl = globalThis.fetch, externalSignal) {
